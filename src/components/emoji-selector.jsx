...
import Emojis from '../assets/enums/emoji';
...
const EmojiSelector = styled.div`
  ...
  border: ${(props) =>
    props.error
      ? '2px solid rgba(255, 72, 72, 0.733)'
  ...
  return (
    <EmojiSelector error={error}>
      {selected &&
        selected.map((emoji) => (
          <EmojiContainer>
            <EmojiDismiss onClick={(e) => onEmojiRemoved(emoji)}>
              {'\u00d7'}
            </EmojiDismiss>
        ...
        target={
          <TooltipEmojiContainer>
            <span style={{ fontSize: '1.3em' }}>
              {/* <img src={AddMoodIcon} alt="addmood" /> */}+
            </span>
            <br />
        ...
        content={
          <TooltipContentWrapper>
            <TooltipContentHeader>
              {hovered
                ? `${hovered.replace(/^\w/, (c) => c.toUpperCase())}`
                : 'Mood'}
            </TooltipContentHeader>
            <TooltipContentBody>
              {Object.keys(Emojis).map((k) => (
                <TooltipContentEmojiWrapper
                  key={k}
                  onClick={(e) => addEmoji(Emojis[k])}
                  onMouseOut={(e) => onHover(null)}
                  onMouseOver={(e) => onHover(Emojis[k].display)}
                >
